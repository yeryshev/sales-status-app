...
import { Box, Button, TextField, Card, Paper } from '@mui/material';
...
export default function StatusBox() {
    ...
    (statusId: number | null) => {
      if (user) {
        dispatch(updateUser({ ...user, commentId: statusId })).then(() => {
          socket.emit('change-comment', { userId: user.id, statusId: 2 });
        });
      }
    },
  ...
  return (
    <FormControl sx={{ width: '100%', gap: 1 }}>
      <Card style={{ margin: '10px', padding: '10px' }}>🟡 Ты теперь НЕ онлайн</Card>
