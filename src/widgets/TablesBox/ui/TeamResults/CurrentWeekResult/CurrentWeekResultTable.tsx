...
export const CurrentWeekResultTable = memo((props: TeamTableProps) => {
  ...
  const filterTeamList = (teammate: Teammate) => {
    ...
    return teammate.secondName && teammate.firstName && Number(getDeals) >= 0;
  };

  const sortTeamList = (a: Teammate, b: Teammate) => {
    const getBudgetA = tasks[a.insideId]?.budget || 0;
    const getBudgetB = tasks[b.insideId]?.budget || 0;

    return Number(getBudgetB) - Number(getBudgetA);
  };
  ...
  return (
    <TableContainer style={{ overflowX: 'auto' }}>
      <Table size="small">
        <caption>Рейтинг менеджеров по новым клиентам за текущую неделю</caption>
        <TableHead>
          <TableRow>
            <TableCell></TableCell>
