...
def upgrade():
    op.alter_column(
        "user", "created_at", server_default=sa.text("TIMEZONE('utc', now())")
    )
    op.alter_column(
        "user", "updated_at", server_default=sa.text("TIMEZONE('utc', now())")
    )
    op.alter_column(
        "status", "created_at", server_default=sa.text("TIMEZONE('utc', now())")
    )
    op.alter_column(
        "status", "updated_at", server_default=sa.text("TIMEZONE('utc', now())")
    )
    op.alter_column(
        "comment", "updated_at", server_default=sa.text("TIMEZONE('utc', now())")
    )
    op.drop_table("task")
...
def downgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "status_id", sa.Integer, sa.ForeignKey("status.id", ondelete="CASCADE")
        ),
        sa.Column(
            "comment_id",
            sa.Integer,
